...
public class Badges {
	...
	public enum Badge {
		...
		BAG_BOUGHT_MATERIAL_SATCHEL,
		BAG_BOUGHT_TRINKET_BAG,
		BAG_BOUGHT_KEY_HOLDER,
		BAG_BOUGHT_ARTIFACT_BAG,
		BAG_BOUGHT_FOOD_BAG,
		ALL_BAGS_BOUGHT             ( 67 ),
		MASTERY_COMBO               ( 68 ),
		MONSTERS_SLAIN_5            ( 69 ),
		...
		CHAMPION_2                  ( 126 ),
		CHAMPION_3                  ( 127 ),
		STAT_RESEARCHER_1           ( 22, BadgeType.JOURNAL ),
		STAT_RESEARCHER_2           ( 55, BadgeType.JOURNAL ),
		STAT_RESEARCHER_3           ( 85, BadgeType.JOURNAL ),
		STAT_RESEARCHER_4           ( 123, BadgeType.JOURNAL );
	...
	public static void validateAllBagsBought( Item bag ) {
		...
		if (bag instanceof VelvetPouch) {
			...
			badge = Badge.BAG_BOUGHT_MATERIAL_SATCHEL;
		} else if (bag instanceof TrinketBag) {
			badge = Badge.BAG_BOUGHT_TRINKET_BAG;
		} else if (bag instanceof KeyHolder) {
			badge = Badge.BAG_BOUGHT_KEY_HOLDER;
		} else if (bag instanceof ArtifactBag) {
			badge = Badge.BAG_BOUGHT_ARTIFACT_BAG;
		...
		if (badge != null) {
			...
			if (!local.contains( Badge.ALL_BAGS_BOUGHT ) &&
				...
				local.contains( Badge.BAG_BOUGHT_MATERIAL_SATCHEL ) &&
				local.contains( Badge.BAG_BOUGHT_TRINKET_BAG ) &&
				local.contains( Badge.BAG_BOUGHT_KEY_HOLDER ) &&
				local.contains( Badge.BAG_BOUGHT_ARTIFACT_BAG ) &&
	...
	public static void validateCatalogBadges(){
		...
		totalSeen += Catalog.totalSeenRarityStats();
		totalThings += Catalog.totalRarityStats();
		...
		int statsSeen = Catalog.totalSeenRarityStats();
		int totalStats = Catalog.totalRarityStats();
		if (statsSeen >= 10) {
			statBadge = Badge.STAT_RESEARCHER_1;
		}
		if (statsSeen >= 25) {
			unlock(statBadge);
			statBadge = Badge.STAT_RESEARCHER_2;
		}
		if (statsSeen >= 50) {
			unlock(statBadge);
			statBadge = Badge.STAT_RESEARCHER_3;
		}
		if (totalStats > 0 && statsSeen == totalStats) {
			unlock(statBadge);
			statBadge = Badge.STAT_RESEARCHER_4;
		}
		displayBadge( statBadge );
	...
	private static final Badge[][] tierBadgeReplacements = new Badge[][]{
			...
			{Badge.STAT_RESEARCHER_1, Badge.STAT_RESEARCHER_2, Badge.STAT_RESEARCHER_3, Badge.STAT_RESEARCHER_4}
	};
