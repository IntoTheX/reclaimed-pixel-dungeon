...
import com.erebus.reclaimedpixeldungeon.actors.buffs.MagicalSight;
import com.erebus.reclaimedpixeldungeon.actors.buffs.MindVision;
import com.erebus.reclaimedpixeldungeon.actors.buffs.RevealedArea;
...
import com.erebus.reclaimedpixeldungeon.actors.hero.spells.DivineSense;
...
import com.erebus.reclaimedpixeldungeon.levels.CavesBossLevel;
import com.erebus.reclaimedpixeldungeon.levels.CavesLevel;
...
import com.erebus.reclaimedpixeldungeon.levels.PrisonLevel;
import com.erebus.reclaimedpixeldungeon.levels.RegularLevel;
import com.erebus.reclaimedpixeldungeon.levels.SewerBossLevel;
import com.erebus.reclaimedpixeldungeon.levels.SewerLevel;
...
public class Dungeon {
	...
	public static final int RAID_THREAT_CHEST_OPENED = 6;
	...
	public static void startNewExpedition() {
		...
		if (hero != null && hero.heroClass != null) {
			hero.heroClass.grantInitialKit( hero );
			hero.updateHT( false );
			hero.HP = hero.HT;
		}
	}
	...
	public static int mobThreat() {
		return mobLevelPressure();
	}

	public static void increaseMobThreat( int amount ) {
		increaseMobLevelPressure( amount );
	}

	public static void resetMobThreat() {
		resetMobLevelPressure();
	}

	public static int raidThreat() {
		return raidThreat;
	}

	public static int raidThreatTarget() {
		if (raidThreatTarget <= 0) rollNextRaidThreatTarget();
		return raidThreatTarget;
	}

	public static void increaseRaidThreat( int amount ) {
		if (amount <= 0 || depth <= 0) return;
		raidThreat = Math.min( 99999, raidThreat + amount );
	}

	public static boolean raidThreatReady() {
		return raidThreat >= raidThreatTarget();
	}

	public static int consumeRaidThreatForRaid() {
		int target = raidThreatTarget();
		int targetRemainder = target % 10;
		int consumed = Math.max( 10, target - targetRemainder );
		raidThreat = Math.max( 0, raidThreat - consumed + targetRemainder );
		return target;
	}

	public static void rollNextRaidThreatTarget() {
		raidThreatTarget = Random.IntRange( RAID_THREAT_TARGET_MIN, RAID_THREAT_TARGET_MAX );
	}

	public static void resetRaidThreat() {
		raidThreat = 0;
		rollNextRaidThreatTarget();
	}
	...
	public static Level newLevel() {
		...
		if (!(level instanceof DeadEndLevel || level instanceof VaultLevel)){
			...
			if (!generatedLevels.contains(depth + 1000*branch)) {
				...
				if (depth > 0) {
					increaseRaidThreat( RAID_THREAT_FLOOR_EXPLORED );
				}
			}
	...
	public static void switchLevel( final Level level, int pos ) {
		...
		Actor.init();
		hero.reactivateEquipmentBuffs( hero.belongings.lostInventory() );
	...
	private static final String MOB_THREAT	= "mob_threat";
	...
	public static void saveGame( int save ) {
		try {
			...
			bundle.put( MOB_THREAT, mobLevelPressure );
