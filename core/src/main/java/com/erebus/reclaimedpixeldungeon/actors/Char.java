...
package com.erebus.reclaimedpixeldungeon.actors;
...
import com.erebus.reclaimedpixeldungeon.actors.buffs.FrostImbue;
import com.erebus.reclaimedpixeldungeon.actors.buffs.Fury;
...
import com.erebus.reclaimedpixeldungeon.actors.buffs.Speed;
import com.erebus.reclaimedpixeldungeon.actors.buffs.Stamina;
...
import com.erebus.reclaimedpixeldungeon.actors.hero.spells.BeamingRay;
...
import com.erebus.reclaimedpixeldungeon.actors.hero.spells.ShieldOfLight;
...
import com.erebus.reclaimedpixeldungeon.items.weapon.missiles.darts.ShockingDart;
...
public abstract class Char extends Actor {
	...
	public static boolean hit( Char attacker, Char defender, float accMulti, boolean magic ) {
		...
		if (defender instanceof Mob) {
			defStat = ((Mob)defender).applyMobStatEvasion( defStat );
		}
	...
	public void damage( int dmg, Object src ) {
		...
		if (this.buff(Doom.class) != null && !isImmune(Doom.class)){
			float doomAmplification = 0.67f;
			if (this instanceof Hero) {
				doomAmplification *= Math.max( 0f, 1f - ((Hero) this).belongings.equippedRarityResistance( Doom.class ) / 100f );
			}
			damage *= 1f + doomAmplification;
		}
		...
		if (sprite != null) {
			...
			if (hitMissIcon == FloatingText.HIT_SUPR) hitMissIcon = -1;
	...
	public void move( int step, boolean travelling ) {
		...
		if (travelling && Dungeon.level.adjacent( step, pos ) && buff( Vertigo.class ) != null) {
			...
			boolean[] vertigoPassable = modifyPassable(
					BArray.or( Dungeon.level.passable, Dungeon.level.avoid, null ) );
			if (!Dungeon.level.insideMap( newPos ) || !vertigoPassable[newPos]
	...
	public boolean[] modifyPassable( boolean[] passable){
		...
		if (Dungeon.level instanceof HomebaseLevel) {
			((HomebaseLevel)Dungeon.level).applyHomebaseStructurePassability( this, passable );
		}
		return passable;
	}
	...
	public float resist( Class effect ){
		...
		if (this instanceof Mob) {
			result *= ((Mob)this).mobStatResistance( effect );
		}
