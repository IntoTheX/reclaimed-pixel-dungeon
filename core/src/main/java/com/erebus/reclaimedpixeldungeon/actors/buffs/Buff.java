...
public class Buff extends Actor {
	...
	public static<T extends FlavourBuff> T append( Char target, Class<T> buffClass, float duration ) {
		float resistance = target.resist(buffClass);
		float resistedDuration = duration * resistance;
		if (resistsDuration( target, buffClass, duration, resistance, resistedDuration )) {
			return Reflection.newInstance(buffClass);
		}
		T buff = append( target, buffClass );
		buff.spend( resistedDuration );
		return buff;
	}
	...
	public static<T extends FlavourBuff> T prolong( Char target, Class<T> buffClass, float duration ) {
		...
		T buff = affect( target, buffClass );
		buff.postpone( resistedDuration );
		return buff;
	}

	private static boolean resistsDuration( Char target, Class<? extends FlavourBuff> buffClass, float duration, float resistance, float resistedDuration ) {
		boolean resisted = resistedDuration <= 0f
				|| (Roots.class.isAssignableFrom( buffClass ) && resistedDuration < TICK);
		if (resisted && duration > 0f && (resistance <= 0f || Roots.class.isAssignableFrom( buffClass ))) {
			showResisted( target );
		}
		return resisted;
	}

	public static void showResisted( Char target ) {
		if (target != null && target.sprite != null) {
			target.sprite.showStatus( CharSprite.POSITIVE, Messages.titleCase(Messages.get(Char.class, "immune")) );
		}
	}
