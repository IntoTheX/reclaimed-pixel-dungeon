...
import com.erebus.reclaimedpixeldungeon.actors.buffs.Combo;
import com.erebus.reclaimedpixeldungeon.actors.buffs.Drowsy;
import com.erebus.reclaimedpixeldungeon.actors.buffs.Foresight;
import com.erebus.reclaimedpixeldungeon.actors.buffs.GreaterHaste;
import com.erebus.reclaimedpixeldungeon.actors.buffs.HeroDisguise;
import com.erebus.reclaimedpixeldungeon.actors.buffs.HoldFast;
...
import com.erebus.reclaimedpixeldungeon.actors.buffs.Invulnerability;
import com.erebus.reclaimedpixeldungeon.actors.buffs.Levitation;
import com.erebus.reclaimedpixeldungeon.actors.buffs.LostInventory;
import com.erebus.reclaimedpixeldungeon.actors.buffs.MindVision;
import com.erebus.reclaimedpixeldungeon.actors.buffs.Momentum;
import com.erebus.reclaimedpixeldungeon.actors.buffs.MonkEnergy;
...
import com.erebus.reclaimedpixeldungeon.actors.hero.spells.HallowedGround;
...
public class Hero extends Char {
	...
	public int STR() {
		...
		if (Dungeon.homebase != null) {
			strBonus += Dungeon.homebase.trainingBonus( HomebaseState.Training.STRENGTH );
		}
	...
	public int bonusTalentPoints(int tier){
		...
		if (Dungeon.homebase != null) {
			switch (tier) {
				case 1:
					bonus += Dungeon.homebase.trainingBonus( HomebaseState.Training.TALENT_POINT );
					break;
				case 2:
					bonus += Dungeon.homebase.trainingBonus( HomebaseState.Training.TALENT_TIER_2 );
					break;
				case 3:
					bonus += Dungeon.homebase.trainingBonus( HomebaseState.Training.TALENT_TIER_3 );
					break;
				case 4:
					bonus += Dungeon.homebase.trainingBonus( HomebaseState.Training.TALENT_TIER_4 );
					break;
			}
		}
	...
	public int attackSkill( Char target ) {
		...
		int trainedAttackSkill = attackSkill;
		if (Dungeon.homebase != null) {
		...
		if (!RingOfForce.fightingUnarmed(this)) {
			...
			return Math.max(1, Math.round(trainedAttackSkill * accuracy));
		}
	}
	...
	public int defenseSkill( Char enemy ) {
		...
		if (Dungeon.homebase != null) {
			evasion += Dungeon.homebase.trainingBonus( HomebaseState.Training.EVASION );
		}
	...
	public float speed() {
		...
		speed *= 1f + belongings.equippedRarityStat( RarityStat.Type.MOVEMENT_SPEED ) / 100f;
		if (Dungeon.homebase != null) {
			speed *= 1f + Dungeon.homebase.trainingBonus( HomebaseState.Training.MOVEMENT_SPEED ) / 100f;
		}
	...
	public float attackDelay() {
		...
		if (!RingOfForce.fightingUnarmed(this)) {
			...
			return applyGlobalAttackSpeed( delay * belongings.attackingWeapon().delayFactor( this ), belongings.attackingWeapon() );
			...
			return applyGlobalAttackSpeed( delay/speed, null );
		}
	}

	private float applyGlobalAttackSpeed( float delay, KindOfWeapon attackingWeapon ) {
		int globalAttackSpeed = belongings.equippedRarityStat( RarityStat.Type.ATTACK_SPEED );
		if (attackingWeapon != null && (attackingWeapon == belongings.weapon() || attackingWeapon == belongings.secondWep())) {
			globalAttackSpeed -= attackingWeapon.rarityStat( RarityStat.Type.ATTACK_SPEED );
		}
		if (Dungeon.homebase != null) {
			globalAttackSpeed += Dungeon.homebase.trainingBonus( HomebaseState.Training.ATTACK_SPEED );
		}
		return delay / (1f + Math.max( 0, globalAttackSpeed ) / 100f);
	}
	...
	private boolean actTransition(HeroAction.LvlTransition action ) {
		...
		if (rooted) {
			...
			if (Dungeon.depth == 0
					...
					&& transition.destDepth > 0) {
				GLog.w( "The dungeon gate is unsafe while the homebase is under attack." );
				ready();
				return false;
			}
	...
	public int defenseProc( Char enemy, int damage ) {
		...
		if (damage > 0 && Dungeon.homebase != null) {
			if (enemy != null
					&& enemy.isAlive()
					&& Random.Int( 100 ) < Dungeon.homebase.trainingBonus( HomebaseState.Training.THORNS_CHANCE )) {
				int thorns = Math.max( 1, Dungeon.homebase.trainingBonus( HomebaseState.Training.THORNS_DAMAGE ) + Math.round( damage * 0.15f ) );
				enemy.damage( thorns, this );
			}
			if (Random.Int( 100 ) < Dungeon.homebase.trainingBonus( HomebaseState.Training.BARRIER_GUARD )) {
				int shielding = Math.max( 1, Dungeon.homebase.trainingBonus( HomebaseState.Training.BARRIER_POWER ) + Math.round( damage * 0.20f ) );
	...
	private boolean homebaseAllyGatePassable( int cell ) {
		return Dungeon.level instanceof HomebaseLevel
				&& ((HomebaseLevel)Dungeon.level).canAllyUseGate( this, cell );
	}

	private boolean homebaseStructureBlocksMovement( int cell ) {
		return Dungeon.level instanceof HomebaseLevel
				&& ((HomebaseLevel)Dungeon.level).blocksStructureMovement( this, cell );
	}

	private boolean homebaseStructureAllowsMovement( int cell ) {
		return Dungeon.level instanceof HomebaseLevel
				&& ((HomebaseLevel)Dungeon.level).allowsStructureMovement( this, cell );
	}
		...
		if (Dungeon.level.adjacent( pos, target )) {
			...
			if (Actor.findChar( target ) == null) {
				if (!homebaseStructureBlocksMovement( target )
						...
						|| homebaseAllyGatePassable( target )
						|| homebaseStructureAllowsMovement( target ))) {
					step = target;
				}
			...
			if (path == null || path.isEmpty() || !Dungeon.level.adjacent(pos, path.getFirst()))
				...
				if (homebaseStructureBlocksMovement( path.get(0) )
						|| (!Dungeon.level.passable[path.get(0)]
						&& !homebaseAllyGatePassable( path.get(0) )
						&& !homebaseStructureAllowsMovement( path.get(0) ))
						|| Actor.findChar(path.get(0)) != null) {
					newPath = true;
				}
			}
	...
	public void earnExp( int exp, Class source ) {
		...
		if (levelUp) {
			...
			if (spiritBow != null) {
				spiritBow.checkRarityStatsForHeroLevel( this );
			}
	...
	public void die( Object cause ) {
		...
		if (Dungeon.depth > 0) {
			returnToHomebaseAfterDeath( cause );
			return;
		}
	...
	private void returnToHomebaseAfterDeath( Object cause ) {
		interrupt();
		...
		InterlevelScene.returnWipesBelongings = true;
		InterlevelScene.mode = InterlevelScene.Mode.RETURN;
		Game.switchScene( InterlevelScene.class );
	}

	public void stripExpeditionMemory( boolean wipeBelongings ) {
		interrupt();
		...
		if (!wipeBelongings) {
			SpiritBow spiritBow = belongings.getItem( SpiritBow.class );
			if (spiritBow != null) {
				spiritBow.preserveLevelForHomebase( this );
			}
		}
		...
		HP = HT;
		live();
		GLog.w( wipeBelongings
				? "The dungeon devours your memories and gear."
				: "The dungeon squeezes away your memories and experience." );
	}
	...
	public void resurrect() {
		...
		reactivateEquipmentBuffs( true );

		updateHT(false);
	}

	public void reactivateEquipmentBuffs( boolean lostInventoryOnly ) {
		...
		for (Item i : belongings){
			...
			} else if (i instanceof CloakOfShadows && (!lostInventoryOnly || i.keptThroughLostInventory()) && hasTalent(Talent.LIGHT_CLOAK)) {
				((CloakOfShadows) i).activate(this);
			} else if (i instanceof HolyTome  && (!lostInventoryOnly || i.keptThroughLostInventory()) && hasTalent(Talent.LIGHT_READING)) {
				((HolyTome) i).activate(this);
			} else if (i instanceof Wand && (!lostInventoryOnly || i.keptThroughLostInventory())){
				if (holster != null && holster.contains(i)){
					((Wand) i).charge(this, MagicalHolster.HOLSTER_SCALE_FACTOR);
				} else {
					((Wand) i).charge(this);
				}
			} else if (i instanceof MagesStaff && (!lostInventoryOnly || i.keptThroughLostInventory())){
