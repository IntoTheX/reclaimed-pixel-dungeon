...
import com.erebus.reclaimedpixeldungeon.actors.buffs.SoulMark;
import com.erebus.reclaimedpixeldungeon.actors.buffs.SwarmIntelTracker;
...
public abstract class Mob extends Char {
	...
	protected final boolean hasMobProgressionContext() {
		return Dungeon.depth > 0
				|| (Dungeon.depth == 0 && Dungeon.homebase != null && Dungeon.homebase.raidActive());
	}

	private boolean shouldRollEliteMob() {
		return alignment == Alignment.ENEMY && hasMobProgressionContext();
	}
	...
	private boolean cellIsPathable( int cell ){
		...
		boolean homebaseAllyGate = Dungeon.level instanceof HomebaseLevel
				&& ((HomebaseLevel)Dungeon.level).canAllyUseGate( this, cell );
		boolean homebaseStructureAllowsMovement = Dungeon.level instanceof HomebaseLevel
				&& ((HomebaseLevel)Dungeon.level).allowsStructureMovement( this, cell );
		if (!Dungeon.level.passable[cell] && !homebaseAllyGate && !homebaseStructureAllowsMovement){
	...
	public int defenseProc( Char enemy, int damage ) {
		...
		if (enemy instanceof HomebaseDefender && isHomebaseRaider()) {
			aggro(enemy);
			target = enemy.pos;
			raidHeroAggro = 5;
		} else if (state != FLEEING) {
			...
			if (enemy == Dungeon.hero && isHomebaseRaider()) {
				raidHeroAggro = 5;
			}
		}
	...
	public void die( Object cause ) {
		...
		boolean homebaseRaidKill = alignment == Alignment.ENEMY
		...
		if (alignment == Alignment.ENEMY && !killedByTranscendantElite){
			...
			if (Dungeon.homebase != null && Dungeon.depth > 0) {
				...
				Dungeon.homebase.recordRaidRevengeKill( getClass() );
				Dungeon.increaseRaidThreat( Dungeon.RAID_THREAT_MOB_KILLED );
			}
		...
		if (homebaseRaidKill) {
			...
			if (cause instanceof HomebaseTowerDefense) {
				...
				((HomebaseLevel)Dungeon.level).reconcileRaidProgress();
			}
		}
	...
	public void rollToDropLoot(){
		...
		if (Random.Float() < catalystChance) {
			Dungeon.level.drop(Generator.randomRarityCatalyst(), pos).sprite.drop();
		}
	...
	private boolean isHomebaseRaider() {
		return alignment == Alignment.ENEMY
				&& Dungeon.homebase != null
				&& Dungeon.depth == 0
				&& Dungeon.level instanceof HomebaseLevel
	...
	public void countInHomebaseRaid() {
		homebaseRaidCounterTracked = true;
	}

	public boolean countsInHomebaseRaid() {
		return homebaseRaidCounterTracked;
	}

	protected void transferHomebaseRaidCountTo( Mob mob ) {
		if (mob == null || !countsInHomebaseRaid() || !isHomebaseRaider()) return;
		mob.countInHomebaseRaid();
		Dungeon.homebase.recordRaidMobReinforcement();
		BossHealthBar.refreshRaid();
	}

	private boolean actHomebaseRaidObjective() {
		if (!isHomebaseRaider()
				|| raidHeroAggro > 0
				|| buff(Amok.class) != null
		...
		HomebaseLevel homebaseLevel = (HomebaseLevel)Dungeon.level;
		if (!Dungeon.homebase.raidHasDamageTargets()) {
		...
		HomebaseState.Building adjacentBuilding = homebaseLevel.raidBuildingAdjacentTo( pos );
		if (adjacentBuilding != null) {
			int impactCell = homebaseLevel.raidBuildingImpactCell( pos, adjacentBuilding );
			if (sprite != null) {
				sprite.attack( impactCell, new Callback() {
					@Override
					public void call() {
						if (sprite != null) {
							sprite.idle();
			...
			int damage = Dungeon.homebase.damageBuilding( adjacentBuilding, Math.max( 1, damageRoll() ) );
			homebaseLevel.showBuildingDamage( impactCell, damage );
			if (damage > 0 && Dungeon.homebase.buildingHP( adjacentBuilding ) <= 0) {
				homebaseLevel.refreshBuildingVisual( adjacentBuilding );
				GLog.w( "Raiders have broken through the " + buildingName( adjacentBuilding ) + "!" );
			}
			spend( attackDelay() );
			return true;
		}

		int attackCell = homebaseLevel.nearestRaidBuildingAttackCell( pos, this );
		if (attackCell == -1) return false;
		target = attackCell;
		int oldPos = pos;
	...
	private String buildingName( HomebaseState.Building building ) {
		String simple = building.name().toLowerCase( java.util.Locale.ENGLISH ).replace( '_', ' ' );
		return simple.substring( 0, 1 ).toUpperCase( java.util.Locale.ENGLISH ) + simple.substring( 1 );
	}
