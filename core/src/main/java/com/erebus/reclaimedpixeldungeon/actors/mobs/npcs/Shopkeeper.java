...
import com.erebus.reclaimedpixeldungeon.actors.buffs.BlobImmunity;
...
import com.erebus.reclaimedpixeldungeon.ui.CurrencyIndicator;
...
public class Shopkeeper extends NPC {
	...
	public static int sellPrice(Item item){
		return item.shopValue() * 5 * (Dungeon.depth / 5 + 1);
	}
	...
	public static boolean canSell(Item item){
		if (item.shopValue() <= 0)                                          return false;
		...
		Game.runOnRenderThread(new Callback() {
			...
			public void call() {
				...
				for (Item item : buybackItems){
					options[i] = Messages.get(Heap.class, "for_sale", item.shopValue(), Messages.titleCase(item.title()));
