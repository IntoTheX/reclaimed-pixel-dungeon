...
public class HomebaseClassFragmentBeacon extends Item {
	...
	public void execute( Hero hero, String action ) {
		...
		if (!HomebaseState.infiniteTestResourcesEnabled()) {
			GLog.w( "Infinite test resources are disabled." );
			return;
		}

		RemainsItem remains = HeroClassUnlocks.debugBossRemainsDrop();
		if (remains == null) {
			GLog.w( "No locked class fragment is eligible to drop." );
			return;
		}

		if (!remains.collect()) {
			if (Dungeon.level != null && hero != null) {
				Dungeon.level.drop( remains, hero.pos ).sprite.drop();
			}
		}
