	...
	public String statsDesc() {
		if (isIdentified()){
			return Messages.get(this, "stats_desc",
					...
					Messages.decimalFormat("#.##", 100*catalystInsteadOfRunestoneChance(buffedLvl())));
		} else {
			return Messages.get(this, "typical_stats_desc",
					Messages.decimalFormat("#.##", 100*stoneInsteadOfSeedChance(0)),
					Messages.decimalFormat("#.##", 100*(grassLootMultiplier(0)-1f)),
					Messages.decimalFormat("#.##", 100*catalystInsteadOfRunestoneChance(0)));
		}
	}
	...
	public static float stoneInsteadOfSeedChance( int level ){
		...
		switch (level){
			...
			case 3:
				return 0.8f;
			default:
				return boundedChance( 0.8f + 0.05f * (level - 3) );
		}
	}

	public static float catalystInsteadOfRunestoneChance() {
		return catalystInsteadOfRunestoneChance( trinketLevel( PetrifiedSeed.class ) );
	}

	public static float catalystInsteadOfRunestoneChance( int level ) {
		if (level < 0) return 0f;
		return boundedChance( 0.15f + 0.05f * level );
	}
}
