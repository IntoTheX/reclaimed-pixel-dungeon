...
import com.erebus.reclaimedpixeldungeon.actors.buffs.ScrollEmpower;
...
public abstract class Wand extends Item {
	...
	public void updateLevel() {
		maxCharges = Math.max( 1, initialCharges() + level() );
		curCharges = Math.min( curCharges, maxCharges() );
	}

	public int maxCharges() {
		int homebaseCharges = Dungeon.homebase == null ? 0 : Dungeon.homebase.trainingBonus( HomebaseState.Training.WAND_CHARGES );
		return Math.max( 1, maxCharges + Math.max( 0, rarityStat( RarityStat.Type.WAND_CHARGES ) ) + Math.max( 0, homebaseCharges ) );
	}
	...
	public class Charger extends Buff {
		...
		private void recharge(){
			...
			float rarityRecharge = 1f + rarityStat( RarityStat.Type.WAND_RECHARGE_RATE ) / 100f;
			if (Dungeon.homebase != null) {
				rarityRecharge *= 1f + Dungeon.homebase.trainingBonus( HomebaseState.Training.WAND_RECHARGE ) / 100f;
			}

			if (Regeneration.regenOn())
				partialCharge += (1f/turnsToCharge) * RingOfEnergy.wandChargeMultiplier(target) * rarityRecharge;
