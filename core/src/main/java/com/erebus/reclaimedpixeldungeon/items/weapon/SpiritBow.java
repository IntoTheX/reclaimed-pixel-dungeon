...
package com.erebus.reclaimedpixeldungeon.items.weapon;
...
public class SpiritBow extends Weapon {
	...
	private int preservedLevel = 0;
	private int rarityCheckedLevel = 0;
	private int mobThreatCheckedLevel = 0;

	private static final String PRESERVED_LEVEL = "preserved_level";
	private static final String RARITY_CHECKED_LEVEL = "rarity_checked_level";
	private static final String MOB_THREAT_CHECKED_LEVEL = "mob_threat_checked_level";
	...
	@Override
	public int level() {
		int level = preservedLevel + heroLevel( Dungeon.hero );
		checkRarityStatsForHeroLevel( Dungeon.hero );
		if (curseInfusionBonus) level += 1 + level/6;
		return level;
	}

	private int heroLevel( Hero hero ) {
		return hero == null ? 0 : hero.lvl/5;
	}

	public void checkRarityStatsForHeroLevel( Hero hero ) {
		int level = preservedLevel + heroLevel( hero );
		trackMobThreatForBowLevel( level );
		rollRarityStatsForBowLevel( level, true );
	}

	public void preserveLevelForHomebase( Hero hero ) {
		int currentRunLevel = heroLevel( hero );
		if (currentRunLevel <= 0) return;
		int level = preservedLevel + currentRunLevel;
		trackMobThreatForBowLevel( level );
		rollRarityStatsForBowLevel( level, true );
		int oldLevel = preservedLevel;
		preservedLevel += currentRunLevel;
		if (preservedLevel != oldLevel) {
			updateQuickslot();
		}
	}

	private void rollRarityStatsForBowLevel( int level, boolean announce ) {
	...
	private void trackMobThreatForBowLevel( int level ) {
		if (level <= mobThreatCheckedLevel) return;
		Dungeon.increaseMobLevelPressure( level - mobThreatCheckedLevel );
		mobThreatCheckedLevel = level;
	}

	@Override
	public void storeInBundle( Bundle bundle ) {
		super.storeInBundle( bundle );
		bundle.put( PRESERVED_LEVEL, preservedLevel );
		bundle.put( RARITY_CHECKED_LEVEL, rarityCheckedLevel );
		bundle.put( MOB_THREAT_CHECKED_LEVEL, mobThreatCheckedLevel );
	}

	@Override
	public void restoreFromBundle( Bundle bundle ) {
		super.restoreFromBundle( bundle );
		preservedLevel = bundle.getInt( PRESERVED_LEVEL );
		rarityCheckedLevel = bundle.getInt( RARITY_CHECKED_LEVEL );
		mobThreatCheckedLevel = bundle.contains( MOB_THREAT_CHECKED_LEVEL )
				? bundle.getInt( MOB_THREAT_CHECKED_LEVEL )
				: preservedLevel;
	}
