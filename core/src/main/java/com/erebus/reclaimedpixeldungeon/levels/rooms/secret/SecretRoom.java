...
public abstract class SecretRoom extends SpecialRoom {
	...
	private static final ArrayList<Class<? extends SecretRoom>> ALL_SECRETS = new ArrayList<>( Arrays.asList(
			...
			SecretCatalystCacheRoom.class, SecretMaterialCacheRoom.class, SecretDefenderRescueRoom.class));

	private static final ArrayList<Class<? extends SecretRoom>> RARE_SECRETS = new ArrayList<>( Arrays.asList(
			SecretLargeCatalystCacheRoom.class, SecretLargeMaterialCacheRoom.class));
	...
	public static void initForRun(){
		...
		for (Class<? extends SecretRoom> rareSecret : RARE_SECRETS) {
			addRareRoomToRunQueue( runSecrets, rareSecret, 0.30f );
		}
	...
	public static void restoreRoomsFromBundle( Bundle bundle ) {
		...
		if (bundle.contains( ROOMS )) {
			...
			ensureRoomInRunQueue( SecretDefenderRescueRoom.class );
			for (Class<? extends SecretRoom> rareSecret : RARE_SECRETS) {
				addRareRoomToRunQueue( runSecrets, rareSecret, 0.15f );
			}
		} else {
			initForRun();
	...
	private static void ensureRoomInRunQueue( Class<? extends SecretRoom> type ) {
		if (!runSecrets.contains( type )) {
			runSecrets.add( Random.Int( runSecrets.size() + 1 ), type );
		}
	}

	private static void addRareRoomToRunQueue( ArrayList<Class<? extends SecretRoom>> queue,
			Class<? extends SecretRoom> type, float chance ) {
