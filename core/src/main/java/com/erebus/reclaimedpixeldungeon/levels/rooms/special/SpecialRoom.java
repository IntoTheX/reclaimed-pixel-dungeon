...
public abstract class SpecialRoom extends Room {
	...
	private static final ArrayList<Class<? extends SpecialRoom>> CONSUMABLE_SPECIALS = new ArrayList<>( Arrays.asList(
			...
			TrapsRoom.class, CrystalPathRoom.class, LockedCatalystCacheRoom.class, LockedMaterialCacheRoom.class,
			LockedDefenderRescueRoom.class
	) );

	private static final ArrayList<Class<? extends SpecialRoom>> RARE_SPECIALS = new ArrayList<>( Arrays.asList(
			LockedLargeCatalystCacheRoom.class, LockedLargeMaterialCacheRoom.class
	) );
	...
	public static void initForRun() {
		...
		for (Class<? extends SpecialRoom> rareSpecial : RARE_SPECIALS) {
			addRareRoomToRunQueue( runConsSpecials, rareSpecial, 0.30f );
		}
	...
	public static void restoreRoomsFromBundle( Bundle bundle ) {
		...
		if (bundle.contains( ROOMS )) {
			...
			ensureRoomInRunQueue( LockedDefenderRescueRoom.class );
			for (Class<? extends SpecialRoom> rareSpecial : RARE_SPECIALS) {
				addRareRoomToRunQueue( runSpecials, rareSpecial, 0.15f );
			}
		} else {
			initForRun();
	...
	private static void ensureRoomInRunQueue( Class<? extends Room> type ) {
		if (!runSpecials.contains( type )) {
			runSpecials.add( Random.Int( runSpecials.size() + 1 ), type );
		}
	}

	private static void addRareRoomToRunQueue( ArrayList<Class<? extends Room>> queue,
			Class<? extends Room> type, float chance ) {
		if (!queue.contains( type ) && Random.Float() < chance) {
			queue.add( Random.Int( queue.size() + 1 ), type );
		}
	}
