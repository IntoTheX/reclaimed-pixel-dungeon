...
package com.erebus.reclaimedpixeldungeon.mechanics;
...
public class Ballistica {
	...
	private void build( int from, int to, boolean stopTarget, boolean stopChars, boolean stopTerrain, boolean ignoreSoftSolid ) {
		...
		while (Dungeon.level.insideMap(cell)) {
			boolean homebaseStructurePassthrough = ignoresHomebaseStructure( cell );
			...
			if (collisionPos == null && stopTerrain && cell != sourcePos && Dungeon.level.solid[cell] && !homebaseStructurePassthrough) {
	...
	private boolean ignoresHomebaseStructure( int cell ) {
		if (!(Dungeon.level instanceof HomebaseLevel)
				|| !((HomebaseLevel)Dungeon.level).isHomebaseStructureCell( cell )) {
			return false;
		}

		Char source = Actor.findChar( sourcePos );
		return source != null && (source == Dungeon.hero || source.alignment == Char.Alignment.ALLY);
	}
