...
import com.erebus.reclaimedpixeldungeon.ui.RadialMenu;
...
import com.erebus.reclaimedpixeldungeon.windows.WndEnergizeItem;
import com.erebus.reclaimedpixeldungeon.windows.WndInfoItem;
import com.erebus.reclaimedpixeldungeon.windows.WndJournal;
...
public class AlchemyScene extends PixelScene {
	...
	public void create() {
		...
		String energyText = Messages.get(AlchemyScene.class, "energy") + " " + WndHomebaseFacility.compactAmount( Dungeon.energy );
		if (toolkit != null){
			energyText += "+" + WndHomebaseFacility.compactAmount( toolkit.availableEnergy() );
		}
		...
		resultMessage = PixelScene.renderTextBlock( 6 );
	...
	private void combine( int slot ){
		...
		Item result = null;
		boolean trinketUpgrade = recipe instanceof Trinket.UpgradeTrinket;
	...
	private void setResultMessage( String text ) {
