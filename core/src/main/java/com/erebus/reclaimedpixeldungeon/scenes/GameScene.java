...
import com.erebus.reclaimedpixeldungeon.tiles.DungeonWallsTilemap;
import com.erebus.reclaimedpixeldungeon.tiles.FogOfWar;
import com.erebus.reclaimedpixeldungeon.tiles.GridTileMap;
import com.erebus.reclaimedpixeldungeon.tiles.RaisedTerrainTilemap;
import com.erebus.reclaimedpixeldungeon.tiles.TerrainFeaturesTilemap;
import com.erebus.reclaimedpixeldungeon.tiles.WallBlockingTilemap;
import com.erebus.reclaimedpixeldungeon.tiles.WallOcclusionTilemap;
...
import com.erebus.reclaimedpixeldungeon.ui.Tag;
import com.erebus.reclaimedpixeldungeon.ui.TargetHealthIndicator;
import com.erebus.reclaimedpixeldungeon.ui.Toast;
...
import com.erebus.reclaimedpixeldungeon.windows.WndInfoMob;
import com.erebus.reclaimedpixeldungeon.windows.WndInfoPlant;
...
public class GameScene extends PixelScene {
	...
	public void create() {
		...
		switch (InterlevelScene.mode) {
			...
			case FALL:
				if (Dungeon.hero.isAlive()) {
					if (InterlevelScene.enteringExpeditionFromHomebase) {
						InterlevelScene.enteringExpeditionFromHomebase = false;
					} else {
		...
		if (Dungeon.depth == 0 && InterlevelScene.defeatedInDungeonReturn) {
			if (!Document.ADVENTURERS_GUIDE.isPageFound( Document.GUIDE_DIEING )) {
				Document.ADVENTURERS_GUIDE.findPage( Document.GUIDE_DIEING );
			}
			if (!Document.ADVENTURERS_GUIDE.isPageRead( Document.GUIDE_DIEING )) {
				GameScene.flashForDocument( Document.ADVENTURERS_GUIDE, Document.GUIDE_DIEING );
				tutorialFlashed = true;
			}
			InterlevelScene.defeatedInDungeonReturn = false;
		}
		if (!tutorialFlashed && Dungeon.depth == 0 && InterlevelScene.successfulHomebaseReturn) {
			tutorialFlashed = ReclaimedTutorial.flash( Document.GUIDE_RETURNING );
			InterlevelScene.successfulHomebaseReturn = false;
		}
		...
		if (SPDSettings.intro()
			...
			ReclaimedTutorial.queue( Document.GUIDE_HOMEBASE );
		}
		if (SPDSettings.intro()){
		...
		if (!tutorialFlashed
				&& !ReclaimedTutorial.flashPending()
				&& !SPDSettings.intro()
				&& Dungeon.depth == 0
				&& Dungeon.level instanceof HomebaseLevel) {
	...
	private void showPendingHomebaseRaid() {
		if (!(Dungeon.level instanceof HomebaseLevel)
		...
		if (!Dungeon.homebase.raidActive()) {
			try {
				Dungeon.saveAll();
			} catch (IOException e) {
		...
		if (Dungeon.homebase.raidWaveSpawned() == 0) {
			((HomebaseLevel)Dungeon.level).spawnRaidWave();
		}
		BossHealthBar.refreshRaid();
		ReclaimedTutorial.flash( Document.GUIDE_RAIDS );

		if (Dungeon.homebase.consumeRaidPopupPending()) {
			addToFront( new WndMessage( Dungeon.homebase.raidPopupText() ) );
			try {
				Dungeon.saveAll();
			} catch (IOException e) {
				ShatteredPixelDungeon.reportException(e);
	...
	public static boolean canFlashForDocument() {
		return scene != null
				&& scene.menu != null
				&& Dungeon.hero != null
	...
	private static boolean handleHomebaseBuildingTap( Integer cell ){
		if (cell == null
				|| Dungeon.hero == null
				|| Dungeon.level == null
				|| !(Dungeon.level instanceof HomebaseLevel)
