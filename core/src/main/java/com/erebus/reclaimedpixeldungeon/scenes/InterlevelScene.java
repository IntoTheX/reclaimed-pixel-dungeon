...
public class InterlevelScene extends PixelScene {
	...
	private static final float HOMEBASE_LOOSE_ITEM_LOSS_CHANCE = 0.5f;
	...
	public static boolean returnWipesBelongings = false;
	public static boolean enteringExpeditionFromHomebase = false;
	public static boolean successfulHomebaseReturn = false;
	public static boolean defeatedInDungeonReturn = false;
	...
	private void descend() throws IOException {
		...
		if (Dungeon.hero == null) {
			...
			enteringExpeditionFromHomebase = Dungeon.depth == 0 && curTransition.destDepth == 1 && curTransition.destBranch == 0;
			if (enteringExpeditionFromHomebase) {
				Dungeon.startNewExpedition();
			}
			Dungeon.saveAll();
	...
	private void ascend() throws IOException {
		boolean returningHomebase = curTransition.destDepth == 0 && curTransition.destBranch == 0 && Dungeon.depth > 0;
		int recoveredMaterials = 0;
		if (returningHomebase) {
			recoveredMaterials = settleHomebaseReturn( false );
		}
		...
		Dungeon.switchLevel( level, destTransition.cell() );
		if (returningHomebase) {
			finishHomebaseReturn( recoveredMaterials, true );
		}
	}
	
	private void returnTo() throws IOException {
		boolean returningHomebase = returnDepth == 0 && returnBranch == 0 && Dungeon.depth > 0;
		boolean wipeBelongings = returnWipesBelongings;
		int recoveredMaterials = 0;
		if (returningHomebase) {
			recoveredMaterials = settleHomebaseReturn( wipeBelongings );
		}
		...
		if (returningHomebase) {
			finishHomebaseReturn( recoveredMaterials, !wipeBelongings );
		}
	}

	private int settleHomebaseReturn( boolean wipeBelongings ) {
		int recoveredMaterials = 0;
	...
	private int looseHomebaseItemsDisappear() {
		if (!(Dungeon.level instanceof HomebaseLevel) || Dungeon.level.heaps == null) return 0;

		int lost = 0;
		for (Heap heap : new ArrayList<>( Dungeon.level.heaps.valueList() )) {
			if (heap.type != Heap.Type.HEAP) continue;
			for (Item item : heap.items.toArray( new Item[0] )) {
				if (item instanceof LostBackpack) continue;
				if (Random.Float() < HOMEBASE_LOOSE_ITEM_LOSS_CHANCE) {
					lost += Math.max( 1, item.quantity() );
					heap.remove( item );
				}
			}
		}
