...
public class ItemSprite extends MovieClip {
	...
	private float rarityAuraClipW;
	private float rarityAuraClipH;
	...
	public synchronized void aura( int color, float alpha, boolean rays ){
		...
		if (alpha <= 0f) {
			...
			rarityAura.hardlight( color );
			rarityAura.radius( Math.min( 13f, Math.max(width(), height()) * 0.72f + 2f ) );
		}
	}

	public void rarityAuraClip( float x, float y, float width, float height ) {
		rarityAuraClipped = width > 0 && height > 0;
		rarityAuraClipX = x;
		rarityAuraClipY = y;
		rarityAuraClipW = width;
		rarityAuraClipH = height;
	}

	public void clearRarityAuraClip() {
		rarityAuraClipped = false;
	}
	...
	public void draw() {
		...
		if (rarityFlare != null) {
			...
			rarityAura.point(x + width()/2f, y + height()/2f);
			if (rarityAuraClipped) {
				clipRarityAura();
			}
			rarityAura.draw();
	...
	private void clipRarityAura() {
		Camera c = camera();
		if (c == null) return;

		Point topLeft = c.cameraToScreen( rarityAuraClipX, rarityAuraClipY );
		Point bottomRight = c.cameraToScreen( rarityAuraClipX + rarityAuraClipW, rarityAuraClipY + rarityAuraClipH );
		int left = Math.max( c.x, Math.min( topLeft.x, bottomRight.x ) );
		int top = Math.max( c.y, Math.min( topLeft.y, bottomRight.y ) );
		int right = Math.min( Math.round( c.x + c.screenWidth() ), Math.max( topLeft.x, bottomRight.x ) );
		int bottom = Math.min( Math.round( c.y + c.screenHeight() ), Math.max( topLeft.y, bottomRight.y ) );
		...
		Gdx.gl20.glScissor(
				...
				Math.round( (Game.height - bottom) * yScale ),
				Math.round( (right - left) * xScale ),
				Math.round( (bottom - top) * yScale ) );
	}

	private void restoreCameraClip() {
		NoosaScript.get().resetCamera();
		NoosaScript.get().camera( camera() );
	}
