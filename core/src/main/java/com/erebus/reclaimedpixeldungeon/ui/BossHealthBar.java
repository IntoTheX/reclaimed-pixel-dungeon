...
import com.erebus.reclaimedpixeldungeon.effects.particles.BloodParticle;
...
public class BossHealthBar extends Component {
	...
	public BossHealthBar() {
		super();
		visible = active = (boss != null || raidActive());
		instance = this;
	}
		...
		bossInfo = new Button(){
			...
			protected String hoverText() {
				if (boss != null){
					...
					return Dungeon.homebase.raidProgressText();
				}
	...
	private void updateRaid() {
		...
		bleeding = false;
		if (blood != null) blood.on = false;
		if (skull != null) skull.resetColor();

		int alive = Dungeon.homebase.raidAlive();
		int total = Math.max( 1, Dungeon.homebase.raidWaveTotal() );
		float percent = alive / (float)total;
		hp.scale.x = percent;
		shieldHP.scale.x = percent;
	...
	public static void refreshRaid(){
		if (instance != null) {
			ShatteredPixelDungeon.runOnRenderThread(new Callback() {
				@Override
				public void call() {
					instance.visible = instance.active = isAssigned() || raidActive();
					if (raidActive() && boss == null) {
						instance.updateRaid();
					}
					instance.layout();
				}
	...
	private static boolean raidActive() {
		return Dungeon.homebase != null && Dungeon.homebase.raidActive();
	}
