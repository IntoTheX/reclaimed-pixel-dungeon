...
public class ItemSlot extends Button {
	...
	protected void layout() {
		...
		sprite.rarityAuraClip( x, y, width, height );
	...
	public void updateText(){
		...
		if (item.icon != -1 && (item.isIdentified() || (item instanceof Ring && ((Ring) item).isKnown()))){
			...
			if (item.levelKnown){
				...
				if (displayStrength() >= 0 && str > displayStrength()) {
	...
	public void strengthContext( int strength ) {
		strengthContext = strength;
		updateText();
	}

	private int displayStrength() {
		if (strengthContext >= 0) return strengthContext;
		return Dungeon.hero == null ? -1 : Dungeon.hero.STR();
	}
