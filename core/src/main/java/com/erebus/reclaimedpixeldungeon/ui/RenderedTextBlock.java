...
public class RenderedTextBlock extends Component {
	...
	private synchronized void build(){
		...
		for (String str : tokens){
			...
			if ((str.equals("_") || str.equals("**")) && highlightingEnabled){
				...
				inlineColor = addInlineText( str, highlighting, inlineColor );
			}
		}
		layout();
	}

	private int addInlineText( String str, boolean highlighting, int inlineColor ) {
		int index = 0;
		while (index < str.length()) {
			int open = str.indexOf( "@@C", index );
			int close = str.indexOf( "@@CEND@@", index );
			int next;
			if (open == -1) next = close;
			else if (close == -1) next = open;
			else next = Math.min( open, close );

			if (next == -1) {
				addWord( str.substring( index ), highlighting, inlineColor );
				break;
			}

			if (next > index) addWord( str.substring( index, next ), highlighting, inlineColor );

			if (close == next) {
				inlineColor = -1;
				index = close + 8;
			} else if (next + 11 <= str.length() && str.startsWith( "@@", next + 9 )) {
				try {
					inlineColor = Integer.parseInt( str.substring( next + 3, next + 9 ), 16 );
					index = next + 11;
				} catch (NumberFormatException e) {
					addWord( str.substring( next, next + 3 ), highlighting, inlineColor );
					index = next + 3;
				}
			} else {
				addWord( str.substring( next, next + 3 ), highlighting, inlineColor );
	...
	private void addWord( String str, boolean highlighting, int inlineColor ) {
		if (str == null || str.isEmpty()) return;

		RenderedText word = new RenderedText(str, size);

		if (inlineColor != -1) word.hardlight(inlineColor);
