...
import com.erebus.reclaimedpixeldungeon.ui.CustomNoteButton;
import com.erebus.reclaimedpixeldungeon.ui.Icons;
import com.erebus.reclaimedpixeldungeon.ui.QuickRecipe;
...
import com.erebus.reclaimedpixeldungeon.ui.ScrollingListPane;
...
public class WndJournal extends WndTabbed {
	...
	public static class CatalogTab extends Component{
		...
		protected void createChildren() {
			...
			itemButtons[STATS_IDX].icon(new ItemSprite(ItemSpriteSheet.MASTERY));
		...
		public void updateList() {
			...
			} else if (currentItemIdx == STATS_IDX) {
				grid.addHeader("_" + Messages.get(this, "title_stats") + "_ (" + Catalog.totalSeenRarityStats() + "/" + Catalog.totalRarityStats() + ")", 9, true);
				for (ItemRarity rarity : ItemRarity.values()) {
					if (Catalog.totalRarityStats( rarity ) <= 0) continue;
					grid.addHeader("_" + rarity.displayName() + "_ (" + Catalog.totalSeenRarityStats( rarity ) + "/" + Catalog.totalRarityStats( rarity ) + "):");
					addGridStats(grid, Catalog.rarityStats( rarity ));
				}
	...
	private static void addGridStats( ScrollingGridPane grid, Collection<RarityStat.Type> types) {
		for (RarityStat.Type type : types) {
			boolean seen = Catalog.isStatSeen( type );
			ItemSprite sprite = new ItemSprite(ItemSpriteSheet.MASTERY);
			if (seen) {
				sprite.hardlight( type.displayColor() );
			} else {
				sprite.lightness(0f);
			}

			String title = seen ? Messages.titleCase( type.displayName() ) : "???";
			String desc;
			if (seen) {
				String rarity = "@@C" + String.format( "%06X", type.minimumRarity().color() & 0xFFFFFF )
						+ "@@" + type.minimumRarity().displayName() + "@@CEND@@";
				desc = type.description();
				desc += "\n\n" + Messages.get(CatalogTab.class, "stat_min_rarity", rarity);
				desc += "\n" + Messages.get(CatalogTab.class, type.unique() ? "stat_unique" : "stat_repeatable");
				desc += "\n" + Messages.get(CatalogTab.class, type.hasValue() ? (type.percent() ? "stat_percent" : "stat_value") : "stat_no_value");

				RarityStat.Type[] requirements = type.requires();
				if (requirements.length > 0) {
					StringBuilder reqText = new StringBuilder();
					for (RarityStat.Type required : requirements) {
						if (reqText.length() > 0) reqText.append( ", " );
						reqText.append( required.displayName() );
					}
					desc += "\n" + Messages.get(CatalogTab.class, "stat_requires", reqText.toString());
				}
			} else {
				desc = Messages.get(CatalogTab.class, "not_seen_stat");
			}
